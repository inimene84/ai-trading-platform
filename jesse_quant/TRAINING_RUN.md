...
## How live `promotion-status` reports these artifacts

`GET /api/jesse/promotion-status` (admin token required) reads **only**
`QTP_PROMOTION_ARTIFACT_DIR` — a sealed `geometry.json` + `metrics.json` bundle.

| Live response | Meaning |
|---|---|
| `status=unconfigured`, `promoted=false` | Env dir unset. Core Hub / Jesse sidecar path only. GPU rejects under `/root/gpu-artifacts/` are **not** loaded. |
| `verdict=REJECT`, `promoted=false` | Bundle present but a QTP gate failed, or `QTP_PROMOTION_REQUIRED=true` with no files. Fail-closed. |
| `verdict=PROMOTE`, `promoted=true` | Sealed bundle passed. Still do not copy B200 rejects into `jesse-trading/storage/models`. |
| `verdict=SHADOW` | Artifact may log predictions; size from this model stays 0. |

`GET /api/jesse/models` is the ops alias for `/ml-models` and embeds the same
promotion snapshot (`has_promoted_model`, `promotion`). Empty `available_models`
plus `has_promoted_model=false` is the expected live state until a sealed
PROMOTE bundle exists.
