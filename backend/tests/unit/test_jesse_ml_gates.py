    ...
    decision = evaluate_promotion(
        {
            ...
            "prob_backtest_overfitting": 0.344,
            "bullish_recall": 0.22,
            "bearish_recall": 0.71,
        },
        pt_mult=5.5,
...
def test_evaluate_promotion_rejects_non_finite_dsr():
...
def test_evaluate_promotion_pass_requires_both_class_recalls():
    decision = evaluate_promotion(
        {
    ...
    assert "class recall" in missing.reason
...
def test_evaluate_promotion_missing_metrics_fail_closed():
    ...
    assert decision.ok is False
    assert "missing class recall" in decision.reason

    decision = evaluate_promotion(
        {"bullish_recall": 0.22, "bearish_recall": 0.71},
        pt_mult=5.5,
        sl_mult=1.75,
        allow_overfit=False,
    )
...
def test_clip_kelly_for_thin_book():
    ...
    zero, was_clipped = clip_kelly_for_thin_book(0.0, closed_count=5)
    assert was_clipped is False
    assert zero == 0.0
