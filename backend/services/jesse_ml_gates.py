...
def evaluate_promotion(
    ...
    *,
    ...
    if bullish_recall is None or bearish_recall is None:
        if allow_overfit:
            return PromotionDecision(
                ok=True,
                reason="class recall missing; JESSE_ML_ALLOW_OVERFIT override enabled",
                dsr=dsr,
                pbo=pbo,
                pt_mult=pt,
                sl_mult=sl,
        ...
        return PromotionDecision(
            ok=False,
            reason="model metadata missing class recall — fail closed",
            dsr=dsr,
            pbo=pbo,
            pt_mult=pt,
            sl_mult=sl,
...
def clip_kelly_for_thin_book(
    ...
    """Until 30 closed trades exist, cap Kelly at 1.0x. Never raise a zero/negative size."""
    size = float(size_multiplier)
    if closed_count >= min_closed:
        return size, False
    if size <= 0:
        return 0.0, False
    clipped = min(1.0, size)
